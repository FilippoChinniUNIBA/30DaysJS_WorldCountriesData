...
for(let country of countries_data) {
    let temp = JSON.stringify(country, ["name", "languages", "population"]);
    countries_data[countries_data.indexOf(country)] = JSON.parse(temp);
}

function mostSpokenLanguages(arr, count) {
    let res = [];
    for(a of arr) {
        for(l of a.languages) {
            tempIndex = res.findIndex(element => element.lingua === l.toUpperCase());
            res[tempIndex] ? res[tempIndex].count++ : res.push({ lingua: l.toUpperCase(), count: 1 });
        }
    }
    return res.sort((a, b) => (a.count > b.count) ? -1 : 1).slice(0, count);
}

function mostPopulatedCountries(arr, count) {
    return arr.sort((a, b) => (a.population > b.population) ? -1 : 1).slice(0, count);
}

const top10Population = mostPopulatedCountries(countries_data, 10);
const top10Languages = mostSpokenLanguages(countries_data, 10);


/* MODIFICA */

const subtitle = document.querySelector(".subtitle");
subtitle.textContent = `Al momento disponiamo Dati di ${countries_data.length} Paesi`;

const feedback = document.querySelector(".feedback");

const graphTitle = document.querySelector(".graph-title");

const graphCountry = document.querySelector("#country-name");
const graphBar = document.querySelector("#bar-graph");
const graphStat = document.querySelector("#stat");


function displayMostPopulatedCountries() {
    graphTitle.textContent = "10 Paesi più Popolosi";

    graphCountry.innerHTML = "";
    graphBar.innerHTML = "";
    graphStat.innerHTML = "";
    ...
    for(const elem of top10Population) {
        ...
        graphBar.appendChild(barP);
        graphStat.appendChild(statP);
    }
...
function displayMostSpokenLanguages() {
    graphTitle.textContent = "10 Lingue più parlate";
...
function creaBarra(HTMLElem, max, curr) {
    const percentuale = (+curr / +max).toFixed(2);
    HTMLElem.id = "bar";
    HTMLElem.textContent = "000000000000000000000000000000000000";
    HTMLElem.style.width = `${percentuale * 100}%`;
}

const buttonPopulation = document.querySelector(".population");
const buttonLanguages = document.querySelector(".languages");

buttonPopulation.textContent = buttonPopulation.textContent.toUpperCase();
buttonLanguages.textContent = buttonLanguages.textContent.toUpperCase();

buttonPopulation.onclick = displayMostPopulatedCountries;
buttonLanguages.onclick = displayMostSpokenLanguages;
